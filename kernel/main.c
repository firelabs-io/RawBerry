...
void memory_print(memory_entry* memory_table, int32_t size){
    ...
    while (i < size){
        ...
        vga_print_string(" ");
        vga_print_string(memre(memory_table[i].start_addr, memory_table[i].end_addr));  
        vga_print_string("\n");
        i++;
    }
}
...
void launch_kernel(void) {
    ...
    while (1) {
        ...
        if (strEql(strSlice(Keyboard_buffer, 0, 4), "echo") == 0) {
            ...
            memory_info(start_address, last_address);
        }
    }
    return;
}
